...
BME280Data readBME280() {
    ...
    bmeData.temperature = adc_T / 100.0f;  // Temperature in °C (needs calibration for accuracy)

    // Convert raw pressure data
    int32_t adc_P = ((data[0] << 12) | (data[1] << 4) | (data[2] >> 4));
    bmeData.pressure = adc_P / 25600.0f;  // Pressure in hPa

    // Convert raw humidity data
    int32_t adc_H = ((data[6] << 8) | data[7]);
    bmeData.humidity = adc_H / 1024.0f;  // Humidity in %RH
